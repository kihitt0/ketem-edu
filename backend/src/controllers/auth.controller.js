...
const register = async (req, res) => {
  ...
  try {
    ...
    // Create user (email_verified = true for legacy flow)
    let result;
    if (existingUser.rows.length > 0) {
      ...
      result = await client.query(
        `INSERT INTO users (email, password, name, phone, country, date_of_birth, role, email_verified)
  ...
  try {
    ...
    if (userResult.rows.length > 0) {
      ...
      if (userResult.rows.length > 0) {
        ...
        // Create new user (NULL password for OAuth-only users, email_verified = true for Google)
        isNewUser = true;
        const newUserResult = await db.query(
          `INSERT INTO users (email, password, name, google_id, avatar_url, role, email_verified)
           VALUES ($1, NULL, $2, $3, $4, 'student', true)
