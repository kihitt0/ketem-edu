...
const getAllUsers = async (req, res) => {
  try {
    ...
    let query = `
      SELECT id, email, name, role, phone, country, is_active, created_at 
      FROM users 
      WHERE 1=1
    `;
    const params = [];
    ...
    const countQuery = query.replace('SELECT id, email, name, role, phone, country, is_active, created_at', 'SELECT COUNT(*)');
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { email, name, role, phone, country, date_of_birth, avatar_url, is_active } = req.body;
    ...
    const query = `
      ...
      SET ${updates.join(', ')} 
      WHERE id = $${paramIndex}
      RETURNING id, email, name, role, phone, country, date_of_birth, avatar_url, is_active, updated_at
    `;
