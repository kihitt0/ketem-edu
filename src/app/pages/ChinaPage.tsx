...
export const ChinaPage: React.FC = () => {
  ...
  const universities = [
    { name: 'Пекинский университет (北京大学)', city: 'Пекин', tuition: '3,000–5,000 USD/год', programs: ['Инженерия', 'Бизнес', 'Медицина', 'IT'], ranking: '#1 в Китае', rank: 1, image: 'https://world-study.kz/wp-content/uploads/2025/06/ws-pekin-1.jpg' },
    { name: 'Университет Цинхуа (清华大学)', city: 'Пекин', tuition: '3,500–6,000 USD/год', programs: ['Инженерия', 'IT', 'Архитектура'], ranking: '#2 в Китае', rank: 2, image: 'https://world-study.kz/wp-content/uploads/2025/06/ws-ts-q.jpg' },
    { name: 'Фуданьский университет (复旦大学)', city: 'Шанхай', tuition: '3,200–5,500 USD/год', programs: ['Экономика', 'Медицина', 'Журналистика', 'Международные отношения'], ranking: '#3 в Китае', rank: 3, image: 'https://worldofeducation.ru/2016/Fudan%20University%20building.jpg' },
    { name: 'Шанхайский университет Цзяо Тун', city: 'Шанхай', tuition: '3,000–5,800 USD/год', programs: ['Инженерия', 'Бизнес', 'Медицина', 'Биотехнологии'], ranking: '#4 в Китае', rank: 4, image: 'https://mymandarin.ru/wp-content/uploads/2025/04/33976729daihocthuonghai-1711436507712.jpg' },
    { name: 'Чжэцзянский университет (浙江大学)', city: 'Ханчжоу', tuition: '2,800–5,000 USD/год', programs: ['IT', 'Инженерия', 'Медицина', 'Сельское хозяйство'], ranking: '#5 в Китае', rank: 5, image: 'https://grant.chilan.ru/wp-content/themes/yootheme/cache/zhejiang-uni-campus-ef063bc3.jpeg' },
    { name: 'Нанкинский университет (南京大学)', city: 'Нанкин', tuition: '2,500–4,500 USD/год', programs: ['Физика', 'Химия', 'Гуманитарные науки', 'История'], ranking: '#6 в Китае', rank: 6, image: 'https://rustudy.uz/wp-content/uploads/2024/12/825_585_nanjing-university-of-finance-economics-3.jpg' },
  ];

  const profKeywords: Record<string, string[]> = {
    medicine:     ['медицин', 'биотехнолог'],
    engineering:  ['инженери', 'it', 'физик', 'химия', 'авиац'],
    business:     ['бизнес', 'экономик', 'менеджмент', 'финанс'],
    architecture: ['архитектур'],
    law:          ['право'],
    humanities:   ['гуманитарн', 'педагогик', 'психолог', 'международн', 'журналист', 'история', 'сельское'],
  };

  const filtered = universities.filter(uni => {
    if (rankFilter === 'top5'  && uni.rank > 5)  return false;
    if (rankFilter === 'top10' && uni.rank > 10) return false;
    if (rankFilter === 'top15' && uni.rank > 15) return false;
    if (profFilter !== 'all') {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50">
      <section className="relative bg-gradient-to-br from-[#1a1a1a] via-[#2C2C2C] to-[#1a1a1a] text-white py-24 overflow-hidden">
        <div className="absolute inset-0">
          <motion.div className="absolute top-20 right-10 w-96 h-96 bg-red-500/20 rounded-full blur-3xl"
            animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }}
            transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            <img src="https://flagcdn.com/w320/cn.png" alt="Флаг Китая" className="w-32 h-20 mx-auto mb-6 rounded-lg shadow-2xl" />
            <h1 className="text-5xl md:text-6xl font-bold mb-6">Обучение в <span className="text-orange-500">Китае</span></h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">Университеты мирового класса с низкой стоимостью обучения и богатой культурной средой</p>
          </motion.div>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            <h2 className="text-4xl font-bold mb-4">Топ университеты <span className="text-orange-500">Китая</span></h2>
            <p className="text-gray-600 text-lg">Престижные учебные заведения с международным признанием</p>
          </motion.div>

          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-8">
            <div className="flex flex-wrap gap-6">
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Место в Китае</div>
                <div className="flex flex-wrap gap-2">
                  {[{ id: 'all', label: 'Все' }, { id: 'top5', label: 'ТОП 5' }, { id: 'top10', label: 'ТОП 10' }, { id: 'top15', label: 'ТОП 15' }].map(f => (
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              ...
              <button onClick={() => { setRankFilter('all'); setProfFilter('all'); }} className="mt-4 text-orange-500 hover:underline text-sm">Сбросить фильтры</button>
            </div>
          ) : (
            <>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <h2 className="text-4xl font-bold mb-4">Почему <span className="text-orange-500">Китай?</span></h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: <DollarSign className="w-8 h-8" />, title: 'Доступная стоимость', description: 'Низкая плата за обучение и проживание по сравнению с Европой и США' },
              { icon: <Globe className="w-8 h-8" />, title: 'Мировое признание', description: 'Дипломы китайских вузов признаются во всём мире' },
              { icon: <Award className="w-8 h-8" />, title: 'Высокое качество', description: 'Множество университетов в топ-500 мирового рейтинга' },
              { icon: <Users className="w-8 h-8" />, title: 'Культурный опыт', description: 'Погружение в одну из древнейших культур мира' },
              { icon: <GraduationCap className="w-8 h-8" />, title: 'Программы на английском', description: 'Широкий выбор программ на английском языке' },
              { icon: <Calendar className="w-8 h-8" />, title: 'Быстрое зачисление', description: 'Простой процесс поступления и получения визы' },
            ].map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="w-16 h-16 bg-gradient-to-br from-orange-500 to-amber-500 rounded-xl flex items-center justify-center text-white mb-4">{item.icon}</div>
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <p className="text-xl mb-8 opacity-90">Наши эксперты помогут подобрать университет и программу специально для вас</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/login" className="bg-white text-orange-500 px-8 py-4 rounded-xl font-bold hover:bg-gray-100 transition-all shadow-lg">Получить консультацию</Link>
              <Link to="/about" className="bg-transparent border-2 border-white text-white px-8 py-4 rounded-xl font-bold hover:bg-white/10 transition-all">Узнать больше</Link>
            </div>
          </motion.div>
        </div>
      </section>
